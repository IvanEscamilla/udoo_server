...
int32_t dwfnFXAS21002Init()
{
	...
	if (ioctl(gdwGyroFd, I2C_SLAVE_FORCE, FXAS21002_SLAVE_ADDR) < 0)
	{
	...
	if(write( gdwGyroFd, &bDatabyte, 2) <= 0)
	{
		gbpBuffer = strerror(errno);
        printf("%s\n\n", gbpBuffer);
...
int32_t dwfnReadGyroData(SGYRORAWDATA *pGyroData)
{
	...
	if (read( gdwGyroFd, bBuffer, FXAS21002_FULL_READ_LEN) != FXAS21002_FULL_READ_LEN) 
	{
        gbpBuffer =  strerror(errno);
        printf("%s\n\n", gbpBuffer);
		return I2C_ERROR;
